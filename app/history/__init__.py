...
class History:
    def __init__(self):
        path= os.environ.get('CALCULATOR_PATH')
        self.path = path
        self.counter = 1
    ...
    def data_input(self, data):
        ...
        cal_data = pd.DataFrame(data_with_id, columns=['ID','Action Performed', 'input1', 'input2'])
    ...
    def clear(self):
        cal_data = pd.DataFrame([], columns=['ID','Action Performed', 'input1', 'input2'])
        cal_data.to_csv(self.path, index=False)